import"./assets/scroll-8076d2f7.js";import{n as T,a as y,P as D}from"./assets/vendor-6ba419a8.js";const k=document.querySelector(".quote__backend"),B=new Date().toLocaleDateString();async function H(){k.innerHTML=`
    ...
    `;try{return(await y.get("https://energyflow.b.goit.study/api/quote")).data}catch{T.Notify.failure("Something went wrong, try again")}}function q(e,t){return`<p class="quote__backend-text">${t}</p>
          ...
          </li>`).join("");u.insertAdjacentHTML("beforeend",o),J()}).catch(s=>{console.log(s.response.data.message)})}catch(s){console.error("Error:",s),alert("Something went wrong, try again")}}await b(v,p);L(12,h).on("afterMove",({page:e})=>{u.innerHTML="",b(v,e)});function J(){let e=p;const t=document.querySelectorAll(".exercises-list-item");for(const n of t){n.addEventListener("click",s);async function s(i){const o=i.currentTarget.dataset.name,r=i.currentTarget.dataset.filter;I.classList.remove("input-hidden"),await E({filter:r,name:o,page:e}),L(9,w).on("afterMove",async({page:c})=>{u.innerHTML="",await E({filter:r,name:o,page:c})}),n.removeEventListener("click",s)}}}const M=document.querySelectorAll(".exercises-filter-button");for(const e of M)e.addEventListener("click",async function(t){I.classList.add("input-hidden");const n=t.currentTarget.dataset.name;for(const s of M)s.classList.remove("active-btn");t.target.classList.add("active-btn"),v=n,console.log("Clicked the button: ",n),u.innerHTML="",g.dataset.name="",g.textContent="",A.textContent="Exercises",await b(n,p),L(12,h).on("afterMove",({page:s})=>{console.log("Moved",s),console.log("Total Pages Pressed",h),u.innerHTML="",b(v,s)}),console.log(n,p)});async function E({filter:e,name:t,page:n,keyword:s=""}){const o={"Body parts":"bodypart",Muscles:"muscles",Equipment:"equipment"}[e];await y.get(`https://energyflow.b.goit.study/api/exercises?${o}=${t.toLowerCase()}&keyword=${s}&page=${n}&limit=${j}`).then(r=>{let c=r.data.results;w=r.data.totalPages,console.log("totalPagesforCards",w,"musclesResult:",r.data),g.dataset.name=t,g.dataset.filter=e,g.textContent=l(t),A.textContent="Exercises /";const m=c.map(({bodyPart:S,burnedCalories:f,name:x,_id:a,target:d,rating:C})=>`<li class="exercises-item-page2">
          ...
          </li> `).join("");u.innerHTML="",u.insertAdjacentHTML("beforeend",m),O()}).catch(r=>{console.log(r.response.data.message)})}I.addEventListener("submit",async e=>{e.preventDefault();let t=p,n=g.dataset.filter,s=g.dataset.name,i=$.value.trim().toLowerCase();console.log("Text Content",$.value.trim().toLowerCase()),await E({filter:n,name:s,page:t,keyword:i}),L(9,w).on("afterMove",async({page:o})=>{u.innerHTML="",await E({filter:n,name:s,page:o,keyword:i})})});
