import"./assets/header-b6fb7402.js";import{n as l,a as d}from"./assets/vendor-a94eb9aa.js";const n=document.querySelector(".quote__backend"),i=new Date().toLocaleDateString();async function p(){n.innerHTML=`
        <span class="loader"></span>
        <span>We're looking a quote for you...</span>
    `;try{return(await d.get("https://energyflow.b.goit.study/api/quote")).data}catch{l.Notify.failure("Something went wrong, try again")}}function c(e,s){return`<p class="quote__backend-text">${s}</p>
            <p class="quote__backend-author">${e}</p>`}function g(){if(localStorage.getItem("quote")&&localStorage.getItem("savedDate")===i){const{author:e,quote:s}=JSON.parse(localStorage.getItem("quote"));n.innerHTML=c(e,s)}else p().then(({author:e,quote:s})=>{const t={author:e,quote:s};localStorage.setItem("quote",JSON.stringify(t)),localStorage.setItem("savedDate",i),n.innerHTML=c(e,s)}).catch(e=>l.Notify.failure(e.message))}g();document.addEventListener("DOMContentLoaded",function(){const e=document.getElementById("subscriptionForm");e.addEventListener("submit",function(s){if(s.preventDefault(),!e.checkValidity())alert("Please fill in a valid email address.");else{const t=new FormData(e);fetch("https://energyflow.b.goit.study/api-docs/#/Subscriptions",{method:"POST",body:t}).then(r=>{if(!r.ok)throw new Error("Network response was not ok");return r.json()}).then(r=>{alert("We're excited to have you on board! 🎉 Thank you for subscribing to new exercises on Energy Flow. You've just taken a significant step towards improving your fitness and well-being."),console.log("Server response:",r)}).catch(r=>{console.error("Error:",r)})}})});const u=document.getElementById("exercises-search-form"),m=u.querySelector(".exercises-search-input"),o=document.querySelector(".exercises-list-page2");u.addEventListener("submit",async e=>{e.preventDefault();const s=m.value.trim().toLowerCase();try{await f(s)}catch(t){console.error(t)}});async function f(e){try{const t=(await d.get(`https://energyflow.b.goit.study/api/exercises?bodypart=back&muscles=lats&equipment=barbell&keyword=${e}&page=1&limit=10`)).data;if(o.innerHTML="",t.length>0){const r=t.map(a=>`
