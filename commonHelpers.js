import"./assets/scroll-8076d2f7.js";import{n as y,a as I}from"./assets/vendor-6ba419a8.js";const d=document.querySelector(".quote-container"),p=new Date().toLocaleDateString();async function P(){d.innerHTML=`
        <span class="loader"></span>
    `;try{return await I.get("https://energyflow.b.goit.study/api/quote").then(t=>t.data)}catch{y.Notify.failure("Something went wrong, try again")}}function A(){if(localStorage.getItem("quote")&&localStorage.getItem("savedDate")===p){const{author:e,quote:t}=JSON.parse(localStorage.getItem("quote"));d.innerHTML=f(e,t)}else P().then(({author:e,quote:t})=>{const i={quote:t,author:e};localStorage.setItem("quote",JSON.stringify(i)),localStorage.setItem("savedDate",p),d.innerHTML=f(e,t)}).catch(e=>y.Notify.failure(e.message))}function f(e,t){return`
      ...
      `}A();const x="/project-radioactive/assets/symbol-defs-b922d213.svg",u=document.querySelector(".favorite-list");u.addEventListener("click",E);const $=document.querySelector(".list-pages");$.addEventListener("click",N);const k=document.querySelector(".empty-favorite"),v=document.querySelector(".list-pages");window.addEventListener("resize",b);const l=document.querySelector(".first-page"),a=document.querySelector(".second-page"),o=document.querySelector(".third-page"),h="favorite",s=JSON.parse(localStorage.getItem(h))??[],g=s.slice(0,8),m=s.slice(8,16),M=s.slice(16,26);function b(){if(window.innerWidth<768&&s.length>=8){n(g),r(),l.classList.add("current-page");return}else{n(s),v.style.display="none";return}}b();function n(e){if(!s.length)u.innerHTML="",k.style.display="flex",v.style.display="none";else{const t=e.map(({_id:i,bodyPart:c,name:S,target:L,burnedCalories:q,time:w})=>`<li data-id="${i}" class="favopite-item">
            ...
            <svg class="svg-name" width="24" height="24"><use href="${x}#icon-runMan"></use></svg>
